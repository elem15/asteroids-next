...
export default function convertAsteroidsFull(asteroid: AsteroidFull): AsteroidOnClient {
  ...
  const isDanger = (size === 'large' && +asteroid.close_approach_data[0]
    .miss_distance.kilometers < 50000000) ? true : false;
  ...
  return {
    id, name, miss_distance_kilometers, estimated_diameter_max, size,
    isDanger, close_approach_data
  };
}
